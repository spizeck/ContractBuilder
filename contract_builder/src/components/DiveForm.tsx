...
import {Boat, Dive, DiveSlot, Guide, Sighting, Site, Species} from "@/types/diveLogTypes";
...
import {celsiusToFahrenheit, fahrenheitToCelsius, feetToMeters, metersToFeet,} from "@/utils/conversions";
...
export default function DiveForm({
      ...
      <Box p={6}>
        ...
        <VStack spacing={4} align="stretch">
          ...
          <FormControl isRequired>
            ...
            <Select value={diveSlot} onChange={(e) => setDiveSlot(e.target.value as DiveSlot)}>
          ...
          <FormControl isRequired>
            ...
            <Select value={diveGuide} onChange={(e) => setDiveGuide(e.target.value)}>
  ...
  if (step > 0 && step <= maxStep) {
    ...
    const stepSpecies = speciesList.filter(
      (sp) => (sp.step || 1) === currentStep
    );
          ...
          {stepSpecies.map((sp) => (
            <FormControl key={sp.id}>
              ...
              <NumberInput
                min={0}
                value={
                  sightings.find((s) => s.speciesId === sp.id)?.count || 0
                }
  ...
  if (step === maxStep + 1) {
    return (
      <Box p={6}>
        ...
        <Text>
          <b>Max Depth:</b> {displayDepth} {prefs?.units.depth === "feet" ? "ft" : "m"}
        </Text>
        <Text>
          <b>Water Temp:</b> {displayTemp} {prefs?.units.temp === "fahrenheit" ? "°F" : "°C"}
        </Text>
