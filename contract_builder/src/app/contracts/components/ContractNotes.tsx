...
import { addContractNote, getContractNotes, deleteContractNote } from '@/services/payments'
      ...
      <VStack spacing={4} align="stretch">
        ...
        {notes.length === 0 ? (
          ...
          <VStack spacing={3} align="stretch">
            {notes.map((note) => (
              <Box key={note.id} p={4} borderWidth="1px" borderRadius="md" bg="gray.50">
                <HStack justify="space-between" align="start">
                  <VStack align="start" spacing={2} flex={1}>
                    <Text whiteSpace="pre-wrap">{note.text}</Text>
                    <Text fontSize="xs" color="gray.500">
                      {note.createdByName || note.createdBy} • {formatDateTime(note.createdAt)}
                    </Text>
                  </VStack>
                  <IconButton
